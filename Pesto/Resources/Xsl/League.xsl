            ...
            <fo:table width="100%" table-layout="fixed">
              ...
              <fo:table-body>
                <fo:table-row>
                  <fo:table-cell>
                    <fo:block>
                      <xsl:value-of select="/Boris/League/Name"/>
                    </fo:block>
                  </fo:table-cell>
                  <fo:table-cell>
        ...
        <fo:flow flow-name="xsl-region-body">
          <fo:block font-family="Nina" font-size="9pt">
            ...
            <fo:block font-size="12pt" font-weight="bold" margin-top="5mm">
              <xsl:text>Overall Results - After </xsl:text>
              <xsl:value-of select="/Boris/Results/EventCount"/>
              <xsl:choose>
                <xsl:when test="/Boris/Results/EventCount = 1"> Event</xsl:when>
        ...
        <fo:table-body>
          <xsl:for-each select="Table/Rows/Row">
            <xsl:variable name="row-color">
              <xsl:choose>
                <xsl:when test="position() mod 2 = 0">#D6E3BC</xsl:when>
                <xsl:otherwise>#EAF1DD</xsl:otherwise>
              </xsl:choose>
            </xsl:variable>
